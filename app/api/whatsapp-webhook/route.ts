...
export async function POST(request: Request) {
  try {
    ...
    // Si es el número de prueba predeterminado de Facebook, redirigir al número real del admin
    if (isTestRequest) {
    ...
    // 1. Obtener teléfonos de administradores
    ...
    const palabrasClave = ["hola", "saldo", "deuda", "cobro", "pago", "buenos dias", "buenas tardes", "buenas noches", "enviar", "envia", "envie", "aviso", "reporte", "informe"]
    ...
    // ── ADMIN: "reporte" o "informe" → informe de cartera ────────────
    if (isAdmin && esComandoReporte) {
      try {
        ...
        if (data.success) {
          await enviarTextoWhatsApp(senderPhone, `✅ Informe generado. Se encontraron ${data.deudores} deudores en mora.`)
        } else {
    ...
    // ── ADMIN: "enviar 303" / "aviso 101" → aviso de ese apto al admin ──
    ...
    // ── FLUJO GENERAL: PROPIETARIO O ADMIN escribe una palabra clave ──

    // Obtener nombre de la torre
    ...
    // Buscar apartamentos del remitente por teléfono
    ...
    if (errUnidades || !unidades || unidades.length === 0) {
      await enviarTextoWhatsApp(
        senderPhone,
        `👋 ¡Hola! Te has comunicado con la administración de ${nombreTorrePrev}.\n\n⚠️ No encontramos ningún apartamento registrado con tu número (${senderPhone}). Comunícate con la administración para registrar tus datos.`
      )
      return NextResponse.json({ success: true })
    }

    // Configuración de torre y tasas
    ...
    // Si escribió "envia" / "enviar" / "aviso" → enviar imagen del aviso de cobro
    if (esEnvioSolo) {
    ...
    // Para todas las demás palabras clave → enviar resumen de texto del estado de cuenta
    for (const u of unidades) {
      ...
      if (mensualidades) {
        mensualidades.forEach((m: any) => {
          ...
          const fechaVig = new Date(anoVigente, mesesNombres.findIndex(mn => mn.toLowerCase() === mesVigente.toLowerCase()))
